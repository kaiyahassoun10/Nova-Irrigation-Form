...
(function () {
  ...
  $("#printPdf").addEventListener("click", () => {
    ...
    const stationHtml = (state.stations.length ? state.stations : [])
      .map((st) => {
        const rows = (
          st.problems.length ? st.problems : [{ label: "—", qty: 0, price: 0 }]
        )
          .map(
            (p) => `
        <tr>
          ...
          <td class="num">${fmt.format(Number(p.price) || 0)}</td>
          <td class="num">${fmt.format(
        ...
        const photo = st.photo
          ? `<img src="${st.photo}" alt="Station ${st.number} photo"/>`
          : `<div class="no-photo">No photo</div>`;
        const sub = (st.problems || []).reduce(
          (s, p) => s + (+p.qty || 0) * (+p.price || 0),
          0
        );
        return `
        <section class="station">
          <div class="flex-between"><h2 style="margin:0">Station ${
            st.number
          }</h2><div class="subtotal">${fmt.format(sub)}</div></div>
          <table>
            <thead><tr><th>Problem</th><th class="num">Qty</th><th class="num">Price</th><th class="num">Line Total</th></tr></thead>
            <tbody>${rows}</tbody>
          </table>
          <div style="display:flex;gap:10px;margin-top:8px">
            <div class="photo">${photo}</div>
            <div style="flex:1">
              <div style="font-size:12px;color:#374151;margin-bottom:4px">Notes</div>
              <div style="min-height:72px;border:1px solid #e5e7eb;border-radius:8px;padding:6px;white-space:pre-wrap">${esc(
                st.notes
              )}</div>
            </div>
          </div>
    ...
    const html = `<!doctype html><html><head><meta charset="utf-8" /><title>Nova Irrigation – Report</title>
      <style>
        ...
        .kv b{display:inline-block;min-width:145px}
        .summary{margin:8px 0 12px;font-weight:700}
        .station{border:1px solid #e5e7eb;border-radius:12px;padding:10px;margin-bottom:10px;page-break-inside:avoid}
        .subtotal{font-weight:800}
        table{width:100%;border-collapse:collapse}
        th,td{padding:6px;border-top:1px solid #e5e7eb}
        thead th{border-top:none;text-align:left}
        .num{text-align:right;white-space:nowrap}
        .photo img{width:120px;height:120px;object-fit:cover;border:1px solid #e5e7eb;border-radius:8px}
        .no-photo{width:120px;height:120px;display:grid;place-items:center;border:1px dashed #cbd5e1;color:#64748b;border-radius:8px;font-size:12px}
        .grand{margin-top:8px;text-align:right;font-weight:900}
      </style>
    
<style id="nova-branding-lite-client-catalog">
#cc_table { width:100%; border-collapse:collapse; }
#cc_table th, #cc_table td { padding:8px; border-top:1px solid #e5e7eb; text-align:left; }
#cc_table thead th { background:#f8faf8; border-top:none; }
</style>

</head><body>
      <header>
        <div class="title">Nova Irrigation – Field Report</div>
        <img src="${BASE64_LOGO}" alt="Nova Logo" />
      </header>
      <section class="kv">
        <div class="grid">
          <div><b>Client:</b> ${esc(state.client.clientName)}</div>
          <div><b>Job #:</b> ${esc(state.client.jobNumber)}</div>
          <div><b>Job Name:</b> ${esc(state.client.jobName)}</div>
          <div><b>Date:</b> ${esc(state.client.date)}</div>
          <div><b>Technician:</b> ${esc(state.client.technician)}</div>
          <div><b>Phone:</b> ${esc(state.client.clientPhone)}</div>
          <div><b>Address:</b> ${esc(state.client.address)}</div>
          <div><b>City/State/Zip:</b> ${esc(state.client.cityStateZip)}</div>
          <div><b>Status (Controller):</b> ${esc(
            state.client.statusOfController
          )}</div>
          <div><b>Status (Backflow):</b> ${esc(
            state.client.statusOfBackflow
          )}</div>
        </div>
      </section>
      <div class="summary">Grand Total: ${total}</div>
      ${stationHtml || '<div style="color:#666">No stations recorded.</div>'}
      <div class="grand">Grand Total: ${total}</div>
...
function initClientPresetControls() {
  ...
  if (sel) {
    sel.addEventListener("change", () => {
      const idx = sel.value;
      if (idx === "") return;
      const it = loadClientCatalog()[parseInt(idx)];
      if (!it) return;
      const map = {
        jobName: "jobName",
